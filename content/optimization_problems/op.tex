...
Vedere la funzione come cerci concentrici in base alla pendenza
...
% \begin{tikzpicture}
%     \def\myfunction(#1,#2){cos(deg(#1)*0.8)*cos(deg(#2)*0.6)*exp(0.1*(#1))+3}
%     \def\scalingfactor{0.2} % Adjust for visual scale
   
  
%     \foreach \i in {0, 5, ..., 10} % Angle in degrees
%     {
%     \pgfmathsetmacro{\radius}{\scalingfactor * \myfunction(\i,0)}
%     \draw (0,0) +(\i:\radius) circle (1pt); % Draw a small circle
%     }
%    \end{tikzpicture}
