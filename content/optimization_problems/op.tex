...
\simone{TODO: mettere dei disegni per gli esempi precedenti.}
...
\begin{outline}
    ...
    \begin{tikzpicture}
        ...
        % builded box
        \draw[fill=boxcolor, line width=1pt, rounded corners=0.125px] (10,3) rectangle (12, 4);
        \draw[fill=boxcolor, line width=1pt, rounded corners=0.125px] (12,3) -- (13, 4) -- (13, 5) -- (12, 4) -- (12, 3);
        \draw[fill=boxcolor, line width=1px, rounded corners=0.125px] (10, 4) -- (12, 4) -- (13, 5) -- (11, 5) -- (10, 4);
...
In one dimension, it is possible to introduce the concept of derivative, which indicates the slope of the function point by point (see \Cref{sec:op.derivative}).
...
\begin{center}
    ...
    \begin{tikzpicture}[remember picture]
        \begin{axis}[
              ...
              xlabel=\( X_2 \),
              zlabel=\( Y \),
              % x axis line style={yshift=-1cm},
              axis line style ={very thick},
              declare function={f(\x,\y)=cos(deg(\x)*0.8)*cos(deg(\y)*0.6)*exp(0.1*\x)+3;}
          ]
              \addplot3[surf, opacity=0.6, domain=0:8]{f(x,y)};
              ...
              \node at (axis cs:2,0,0) [anchor=north, color=black] {\( x_2 \)};
              \node at (axis cs:0,2,0) [anchor=south east, color=black] {\( x_1 \)};
              \node at (axis cs:2,2,0) [anchor=west, color=black] {\( x \)};
              ...
              \node at (axis cs:2,2,2) [anchor=west, color=black] {\( y = f(x) = f(x_1, x_2) \)};

              \draw[arrow, myblue, line width=0.5pt] (0,0) -- (2,2);
              \node at (axis cs:1,1,0) [anchor=south, color=black] {\( \vec{x} \)};
...
\begin{minipage}{0.45\linewidth}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \addplot3[
                contour gnuplot={levels={1, 1.25, 1.5, 1.75, 2, 2.25, 2.5, 2.75, 3, 3.25, 3.5, 3.75, 4, 4.25}},
                domain=0:8
            ] {cos(deg(x)*0.8)*cos(deg(y)*0.6)*exp(0.1*x)+3};
            \end{axis}
            \end{tikzpicture}
    \end{center}
\end{minipage}
...
\paragraph{Gradient.} As with one-variable functions, we want to know the slope of the function at a given point; to do this, we introduced the concept of derivative. For functions with two (or more) dimensions, we use the concept of gradient \( \nabla f(x_1, x_2) \). 
...
{   
    ...
    \begin{itemize}
        ...
        \begin{tikzpicture}[overlay,remember picture]
            \node (X1h_desc) [above of = X1h, node distance = 3em, right=1em, anchor=west] {\footnotesize \textsf{\textcolor{myred}{calculates how the value of the function changes when \( x_1 \) is varied slightly}}};
            \draw[<-, in=180, out=90] (X1h.south) ++ (1em,0.5em) to (X1h_desc.west);
        
            \node (X2_desc) [above of = X2, node distance = 2em, right=.5em, anchor=west] {\footnotesize \textsf{\textcolor{mygreen}{treat \( x_2 \) as a constant}}};
            \draw[<-, in=180, out=90] (X2.south) ++ (.25em,0.5em) to (X2_desc.west);
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[
                ...
                axis line style ={very thick},
                declare function={
                    f(\x,\y)=cos(deg(\x)*0.8)*cos(deg(\y)*0.6)*exp(0.1*\x)+3;
                    dfX(\x,\y) = (-0.8*sin(deg(\x)*0.8)*cos(deg(\y)*0.6) + 0.1*cos(deg(\x)*0.8)*cos(deg(\y)*0.6)) * exp(0.1*\x);
                    dfY(\x,\y) = -0.6*cos(deg(\x)*0.8)*sin(deg(\y)*0.6)*exp(0.1*\x);    
                }
            ]
                \addplot3[surf, opacity=0.6, domain=0:8]{f(x,y)};     
                ...
                \addplot3[
                    quiver={
                        u={dfX(x,y)},
                        v={dfY(x,y)},
                        w=0, % since it's a 2D gradient
                        scale arrows=0.5,
                    },
                    -stealth, line width=2pt, black, samples=12, domain=0.5:7.5
                ] {f(x,y)};
        ...
        \begin{tikzpicture}
            ...
            [
                ...
                \addplot3[
                    ...
                    -stealth, line width=1pt, black, samples=12, domain=0.5:7.5
                ] {f(x,y)};
    ...
    \begin{tikzpicture}
        \begin{axis}[
                ...
                \addplot3[
                    ...
                    -stealth, line width=1.5pt, black, samples=12, domain=0.5:7.5
                ] {f(x,y)};
...
\subsection[many dimensions]{\( n \)-dimension}
\label{sec:op.dimensions.many}

Functions in more than two dimensions are difficult (if not impossible) to represent graphically, but it is still possible to extend the concepts seen in \Cref{sec:op.dimensions.two} for two dimensions to an arbitrary number of dimensions.


Let \( n \) be the dimensions, \( \mathbb{R}^n \) the domain, and \( \mathbb{R} \) the co-domain. An element of the domain is a tuple \( x=(x_1, x_2, ..., x_n) \), where \( x \in \mathbb{R}^n \). A function \( f \) in  \( n \) dimensions is \( f : \mathbb{R}^n \rightarrow \mathbb{R} \).
...
\input{content/optimization_problems/lab.tex}
